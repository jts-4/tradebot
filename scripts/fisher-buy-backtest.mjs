// Fisher9 Al Sinyali Backtest
// Sinyal: Fisher < trigger → trigger üstüne geçti + Fisher < 0 (oversold)
...
function backtest(candles, forwardBars) {
  ...
  for (let i = 1; i < fishArr.length - forwardBars; i++) {
    ...
    // Al sinyali: Fisher önceki trigger altında → şimdi üstüne geçti + Fisher < 0
...
async function main() {
  ...
  console.log('\n=== FISHER9 AL SİNYALİ BACKTEST (2H, 1 Yıl) ===')
